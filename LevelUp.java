...
public class LevelUp extends JPanel{
	...
	public void deleteObject(int x, int y) { //delete food item as player eat it
		myObjects[x][y] = null; //empty food item at x,y coordinate
	}
	...
	public void moveObject(int old_X, int old_Y, int new_X, int new_Y) { // create a method for level up
		...
		if (myObjects[new_X][new_Y] != null && myObjects[new_X][new_Y].type != ITEM_TYPE.PLAYER) {
			switch (myObjects[new_X][new_Y].type) { //evaluate the object for switch case
    ...
    public static void displayJFrame() {
		...
		LevelUp arena = new LevelUp(); // create a new Arena for level up xp
    ...
    public void refreshScreen() {
        timer = new Timer(0, new ActionListener() {
          ...
          public void actionPerformed(ActionEvent e) {
        	...
        	while (collision) {      
	        	Random rand = new Random(); // declare random type variable
        ...
        timer.setDelay(2000); // food will spawn every 2 seconds
        timer.start();
      }
}
